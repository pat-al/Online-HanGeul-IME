...
function initialize_options() {
	...
	NCR_option.show_NCR_text = 0;
	NCR_option.convert_only_NFD_hangeul_encoding_in_NCR_text = 0;	
	NCR_option.show_direct_typing_text = 0;	
}
...
function combine_unicode_NFD_hangeul_phoneme(c1,c2) { // 유니코드 한글 낱자 조합하기
	...
	// 이어치기 자판
		...
		// 요즘한글 일반 낱자 조합
		if(!option.enable_old_hangeul_input && typeof current_layout_info.hangeul_combination_table != 'undefined' && typeof current_layout_info.hangeul_combination_table.length != 'undefined' && current_layout_info.hangeul_combination_table.length)
			combination_table = current_layout_info.hangeul_combination_table;

		// 편의를 높이기 위한 낱자 조합을 더함 (옛한글 자판이 아닌 한글 자판)
		if(option.convenience_combination && typeof current_layout_info.hangeul_convenience_combination_table != 'undefined' && !is_old_hangeul_input())
			combination_table = combination_table.concat(current_layout_info.hangeul_convenience_combination_table);

		// 옛한글 낱자 조합
		if(current_layout_info.type_name.substr(-2)=='-y' && typeof current_layout_info.combination_table != 'undefined')
			combination_table = current_layout_info.combination_table;
...
function ohiHangeul3(f,e,key) { // 세벌식 자판 - 낱자 단위 처리
	...
	if(!abbreviation_processing_state) {
		if(typeof current_layout_info.sublayout != 'undefined') {
			sublayout = current_layout_info.sublayout;
			if(checkCapsLock() && typeof current_layout_info.capslock_sublayout != 'undefined') {
				sublayout=current_layout_info.capslock_sublayout;
			}
		}
		if(typeof current_layout_info.extended_sign_layout != 'undefined') extended_sign_layout = current_layout_info.extended_sign_layout;
		if(checkCapsLock() && typeof current_layout_info.capslock_extended_sign_layout != 'undefined') extended_sign_layout = current_layout_info.capslock_extended_sign_layout;
		prev_cursor_position = -1;
	}
	...
	// key가 유니코드 한글 낱자일 때
		...
		if(c==0x1B) c=0; // 글쇠 자리의 부호값이 0x1B이면 0으로 바꿔 뒤에서 조합 끊는 처리를 하게 함
	}
	...
	if(!abbreviation_processing_state || is_moachigi_input()) {
		...
		// 첫가끝 방식으로 조합할 때
			if(Ko_type.substr(0,5)=='Sin3-') { // 옛한글 신세벌식 자판
				c=NFD_Sin3_preprocess(f,e,key);
				if(c==-1) return 0;
			}
			if(c) {
				NFD_hangeul_input(f,key,c); // 첫가끝 조합형으로 옛한글 낱자 처리하기
				return 0;
			}
		}

		if(Ko_type.substr(0,5)=='Sin3-') { // 요즘한글 신세벌식 자판
			c=NFC_Sin3_preprocess(f,e,key);
			if(c==-1) return 0;
		}
...
function NFD_hangeul_input(f,key,c) {	// 첫가끝(세벌식) 부호계를 쓰는 옛한글 처리
	...
	if(!combined_phoneme&&unicode_cheos.indexOf(c)>=0 || unicode_NFD_hangeul_code.indexOf(c)<0) {
		...
		complete_hangeul_syllable(f);
		if(i && is_phonemic_writing_input() && option.phonemic_writing_adding_space_every_syllable_end) ohiInsert(f,0,32); // 풀어쓰기할 때 낱내자 뒤에 빈칸 넣기 (한글 조합이 새로 이어질 때)
	}
	...
	// 앞에 첫소리가 없이 가운뎃소리가 들어왔을 때
		i = unicode_NFD_hangeul_code.indexOf(c)>=0 && NFD_stack.phoneme.length ? 1 : 0;
		complete_hangeul_syllable(f);
		if(i && option.phonemic_writing_adding_space_every_syllable_end && is_phonemic_writing_input()) ohiInsert(f,0,32); // 풀어쓰기할 때 낱내자 뒤에 빈칸 넣기 (한글 조합이 새로 이어질 때)
	...
	// 첫가끝 조합 상태에서 받침이 들어 있는데 또 받침이 들어왔을 때
		if(unicode_ggeut.indexOf(NFD_stack.phoneme[1])<0) {
			...
			// 같은 받침 글쇠가 거듭 눌렸을 때 겹받침 확장 배열 적용하기
				c = convert_into_unicode_hangeul_phoneme(c1);
				NFD_stack.phoneme.unshift(c);
				NFD_stack.phoneme_R.unshift(0);
				NFD_stack.combined_phoneme[0]=convert_into_unicode_hangeul_phoneme(sub_c1);
				ohiSelection(f,0);
				ohiBackspace(f);
				ohiInsert(f,0,convert_into_unicode_hangeul_phoneme(sub_c1));
				ohiSelection(f,NFD_stack.combined_phoneme.length);				
				return -1;
			}
		}
...
function NFD_Sin3_preprocess(f,e,key) { // 첫가끝 방식으로 조합하는 신세벌식 한글 처리 (옛한글)
	...
	// 첫소리가 들어갔을 때에 오른손 자리에 있는 겹홀소리 조합용 가운뎃소리(ㅗ, ㅜ, ㅡ, ㆍ 등) 넣기
		c = -c2;
		if(!(layout_info.type_name=='Sin3-P' && is_old_hangeul_input && checkCapsLock())) c=c1;
	}
	else if(option.enable_Sin3_diphthong_key && !with_shift_key(key) && NFD_stack.phoneme.length && unicode_cheos.indexOf(NFD_stack.phoneme[0])>=0 && sub_c1 && unicode_ga.indexOf(sub_c1)>=0) {
	...
	// 첫소리만 들어갔고, 왼손 쪽의 끝소리가 있는 글쇠가 윗글쇠와 함께 눌렸을 때 끝소리를 넣음 (홀소리만 빠진 미완성 낱내자 조합)
		c = c2;
	}
...
function show_NCR_text(op) { // 문자를 유니코드 부호값과 맞대어 나타내기 (Numeric Character Reference)
	if(typeof op != 'undefined') {
		if(op) NCR_option.show_NCR_text=1;
		else NCR_option.show_NCR_text=0;
	}
	...
	if(opts) {
		...
		var opt = document.getElementById('NCR_option_show_NCR_text');
		if(!opt) opt = appendChild(opts,'div','option','NCR_option_show_NCR_text','<div class="option"><input name="show_NCR_text" class="checkbox" onclick="show_NCR_text(this.checked);inputText_focus()" type="checkbox"' + (NCR_option.show_NCR_text ? ' checked="checked"' : '') + '><label title="&apos;한글&apos;을 &amp;#xD55C;&amp;#xAE00; 꼴로 나타내기">HTML 문자 참조</label></div>');

		opt = document.getElementById('NCR_option_convert_only_NFD_hangeul_encoding_in_NCR_text');
		if(!opt) opt = appendChild(opts,'div','option','NCR_option_convert_only_NFD_hangeul_encoding_in_NCR_text','<div class="option"><input name="convert_only_NFD_hangeul_encoding_in_NCR_text" class="checkbox" onclick="NCR_option.convert_only_NFD_hangeul_encoding_in_NCR_text=this.checked;show_NCR_text();inputText_focus()" type="checkbox"' + (NCR_option.convert_only_NFD_hangeul_encoding_in_NCR_text ? ' checked="checked"' : '') + '><label title="완성형으로 나타낼 수 있는 한글은 바꾸지 않기">첫가끝 조합형만 바꾸기</label></div>');
	}

	if(t && NCR_option.show_NCR_text) {
	...
	for(i=0;i<f.value.length;++i) {
		...
		if(NCR_option.convert_only_NFD_hangeul_encoding_in_NCR_text) {
		// 첫가끝 조합형 한글만 바꿀 때
...
function show_direct_typing_text(op) { // 쿼티 글쇠 배열 기준으로 문자열 바꾸기 (Numeric Character Reference)
	if(typeof op != 'undefined') {
		if(op) NCR_option.show_direct_typing_text=1;
		else NCR_option.show_direct_typing_text=0;
	}
	...
	if(opts) {
		...
		var opt = document.getElementById('NCR_option_show_direct_typing_text');
		if(!opt) opt = appendChild(opts,'div','option','NCR_option_show_direct_typing_text','<div class="option"><input name="show_direct_typing_text" class="checkbox" onclick="show_direct_typing_text(this.checked);inputText_focus()" type="checkbox"' + (NCR_option.show_direct_typing_text ? ' checked="checked"' : '') + '><label title="쿼티 배열 기준 직결식 변환">직결식 변환 (미완성 기능)</label></div>');
	}

	if(t && NCR_option.show_direct_typing_text) {
		t.style.display='inline-block';
	}
	else {
		t.style.display='none';
	...
	var char_code, conv_char, conv_text='';
	for(var i=0;i<f.value.length;++i) {
		char_code = f.value.charCodeAt(i);
		conv_text += convert_into_direct_typing_chars(char_code);
	}
	if(conv_text=='') conv_text='&nbsp;';
	t.innerHTML = conv_text;
}

function convert_into_direct_typing_chars(code) {
	if(typeof code == 'undefined' || code<32) return '';
	var i, text = '', key, chars = [];
	
	if(code<127) {
		chars.push(code);
	}
	else if(code>=0xAC00 && code<=0xD7AF) {	
		var p = convert_NFC_into_NFD(code);
		for(i=0; i<p.length; ++i) {
			if(!p[i]) continue;
			chars.push(current_layout_info.layout.indexOf(p[i])+33);
		}
	}
...
function show_options() {
	...
	if(opts) {
		...
		ft = 'inputText_focus()"><label title="낱내자(음절자) 사이에 빈칸 넣기">낱내 띄기</label></div>';
		opt = add_option(opts, opt_name, ft);
...
function show_keyboard_layout(type) {
	...
	if(sublayout.length && !is_old_hangeul_input()
	 && (option.enable_double_final_ext || current_layout_info.type_name.substr(0,3)=='3m-' || current_layout_info.type_name=='3-18Na' || current_layout_info.type_name=='3-D1')
	 && sign_ext_state<=0) {
	...
	if(KE=='Ko' && Ko_type.substr(0,4)=='Sin3') { // 신세벌식 자판의 첫소리 자리에서 넣는 홀소리들
		if(!sign_ext_state && !(!option.enable_Sin3_diphthong_key && is_old_hangeul_input()) && !(checkCapsLock() && typeof layout_info.capslock_extended_sign_layout != 'undefined' && !layout_info.capslock_extended_sign_layout)) {
			if(sublayout) {
				...
				if(sublayout[72]) // 신세벌식 P2의 오른쪽 ㅡ 자리 (i 자리)
					document.getElementById('de22').innerHTML = '<span style="font-size:10px; letter-spacing:-2px;color:#333;">('+String.fromCharCode(convert_into_compatibility_hangeul_letter(sublayout[72]))+')</span>';
				if(sublayout[78]) // 신세벌식 P2의 오른쪽 ㅜ 자리 (o 자리)
					document.getElementById('de23').innerHTML = '<span style="font-size:10px; letter-spacing:-2px;color:#333;">('+String.fromCharCode(convert_into_compatibility_hangeul_letter(sublayout[78]))+')</span>';
				if(sublayout[79]) {
					if(sublayout[79]==0x119E) // P 자리의 겹낱자 확장 배열 (신세벌식 P2의 오른쪽 아래아)
						document.getElementById('de24').innerHTML = '<span style="font-size:10px; letter-spacing:-3px;color:#333;">('+String.fromCharCode(convert_into_compatibility_hangeul_letter(sublayout[79]))+')</span>';
					else
						document.getElementById('de24').innerHTML = '<font size="1">'+String.fromCharCode(convert_into_compatibility_hangeul_letter(sublayout[79]))+'</font>';
				}
			}
			else {
...
function checkCapsLock() {
	var e=window.event;
  if(e.getModifierState && e.getModifierState("CapsLock")) return true;
  return false;
}
...
function url_query() {
	...
	for(var i=0; i<fields.length; ++i){
		...
		if(field=='kbd') { // 기준 자판
			...
			NCR_option.convert_only_NFD_hangeul_encoding_in_NCR_text = TF;
		}
