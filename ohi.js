...
function ohiHangeul2(f,e,c) { // 2-Beolsik
	...
	if(typeof current_layout.layout != 'undefined') {
		...
		if(cc==current_layout.layout[c-33]) {
			ohiInsert(f,0,cc);
			return;
		}
		...
		// 한글 낱자가 든 글쇠를 윗글쇠와 함께 눌렀을 때
			cc += cc==32||cc==36?2:cc==18||cc==7||cc==24||cc==1||cc==21?1:0;
		}
	}
	...
	if(cc<31) { // Jaum
		...
		if(Ko_type.substr(0,5)=='2-sun' || Ko_type=='2-Gaon26KM') {
...
function ohiHangeul3(f,e,c) { // 세벌식 자판 (3-Beolsik)
	...
	// 신세벌식 자판으로 풀어쓰기 처리
		if( (ohiQ[3] || ohiQ[6]) && (cc>127 && cc<158 && cc!=147 || ohi_hangeul_phoneme.indexOf(cc)<0) ) {
...
function CGG_Hangeul_Sin3(f,c) { // 첫가끝 방식으로 조합하는 신세벌식
	...
	else if(option.enable_Sin3_diphthong_key && no_shift(c) && prev_phoneme.length && unicode_cheot.indexOf(prev_phoneme[0])>=0 && unicode_cheot.indexOf(cc)>=0 && unicode_ga.indexOf(cc2)>=0) {
	// 첫소리가 들어갔을 때에 오른손 자리에 있는 겹홀소리 조합용 가운뎃소리(ㅗ, ㅜ, ㅡ, ㆍ 등) 넣기
		cc=-cc2;
	}
	else if(option.enable_Sin3_diphthong_key && no_shift(c) && prev_phoneme.length && unicode_cheot.indexOf(prev_phoneme[0])>=0 && Sin3_sublayout && unicode_ga.indexOf(convert_into_unicode_hangeul_phoneme(Sin3_sublayout[c-33]))>=0) {
	// 첫소리가 들어갔고 가운뎃소리가 들어가지 않았을 때 보조 배열의 겹홀소리 조합용 홀소리를 넣음
		cc=-Sin3_sublayout[c-33];
	}
...
function show_options() {
	...
	if(opts) {
		...
		if(option.show_layout && !option.enable_Sin3_yeshangeul_combination && typeof current_layout.sublayout != 'undefined' && current_layout.type_name.substr(0,3)!='3m-') opt.style.display = 'block';
		else opt.style.display = 'none';
		...
		if(current_layout.type_name.substr(-2)=='-y' || option.enable_Sin3_yeshangeul_combination&&current_layout.type_name.substr(0,5)=='Sin3-') opt.style.display = 'block';
		else opt.style.display = 'none';
			...
			if(uh[i]) {
				if(uh[i][j]) {
					...
					if(Ko_type.substr(0,2)=='3-' && Number(Ko_type.substr(2,4))>=2014 && unicode_modern_ggeut.indexOf(charCode)>=0 && unicode_modern_hotbatchim.indexOf(charCode)<0) {
						uh[i][j] = '<span style="color:gray;">a'+uh[i][j]+'</span>';
					}
				}
...
function basic_layouts_info() {
	...
	K3_Sin3_P_sublayout = [
		...
		0x11b2,	/* 0x45 E: jongseong lieul-bieub */
		0x11b9,	/* 0x46 F: jongseong bieub-sieus */
		0x0000,	/* 0x47 G */
		0x0000,	/* 0x48 H */
		0x0000,	/* 0x49 I */
		0x0000,	/* 0x4A J */
		...
		0x0000,	/* 0x50 P */
		0x11b3,	/* 0x51 Q: jongseong lieul-sieus */
		0x11b5,	/* 0x52 R: jongseong lieul-pieup */
		0x11ad,	/* 0x53 S: jongseong nieun-hieuh */
		0x0000,	/* 0x54 T */
		0x0000,	/* 0x55 U */
		...
		0x11b2,	/* 0x65 e: jongseong lieul-bieub */
		0x11b9,	/* 0x66 f: jongseong bieub-sieus */
		0x0000,	/* 0x67 g */
		0x0000,	/* 0x68 h */
		0x0000,	/* 0x69 i */
		0x0000,	/* 0x6A j */
		...
		0x0000,	/* 0x70 p */
		0x11b3,	/* 0x71 q: jongseong lieul-sieus */
		0x11b5,	/* 0x72 r: jongseong lieul-pieup */
		0x11ad,	/* 0x73 s: jongseong nieun-hieuh */
		0x0000,	/* 0x74 t */
		0x0000,	/* 0x75 u */
