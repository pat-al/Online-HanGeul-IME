...
var unicode_CGG_hangeul_phoneme = [], unicode_cheot = [], unicode_ga = [], unicode_ggeut=[]; // 유니코드 한글 낱자, 유니코드 한글 첫·가·끝 낱자
...
function seek_abbreviation(abbreviation_table, c1, c2) { // 줄임말 조합을 찾기 (이어치기 자판)
	var i;
	var chars=null;
...
function ohiHangeul3_abbreviation(f,e,c) { // 이어치기 세벌식 자판에서 줄임말 처리
	...
	if(option.abbreviation) abbreviation_table = K3_hangeul_abbreviation_table;
	if(typeof current_layout.hangeul_abbreviation_table != 'undefined') abbreviation_table = current_layout.hangeul_abbreviation_table;	
	if(!abbreviation_table || !abbreviation_table.length) return 0;

	if(abbreviation_table && !prev_phoneme.length) {
		...
		if(Ko_type.indexOf('Sin3')>=0) {	// 신세벌식 자판 또는 바꾼꼴(공세벌식형) 신세벌식 자판
			if(Ko_type.substr(0,5)=='Sin3-') {
				if(option.enable_Sin3_yeshangeul_combination && typeof current_layout.extended_hangeul_combination_table != 'undefined') {
					...
					if(cc==-1) return 0;
					CGG_yesHangeul(f,c,cc); // 첫가끝 조합형으로 옛한글 낱자 처리하기
					return 0;
				}
			}
			cc=Hangeul_Sin3(f,e,c);
			if(cc==-1) return 0;
		}
...
function Hangeul3_sign_ext(f,e,c) {
	...
	if(Ko_type.substr(0,3)=='3m-') {
		cc=current_layout.layout[c-33];
		if(Hangeul_SignExtKey1>0) {
		// 기호 확장 배열에서 기호를 넣음
			cc=current_layout.extended_sign_layout[c-33][Hangeul_SignExtKey1-1];
			ohiInsert(f,0,cc);
			esc_ext_layout();
			return 1;
		}
		...
		// 기호 확장 배열로 들어감
			Hangeul_SignExtKey1=cc;
			show_keyboard_layout();
			return 1;
		}
		else if(Hangeul_SignExtKey1<0) {
			// 기호 확장 배열 상태에서 다음 상태(기호를 넣을 확장 배열)를 결정함
			if(c==75) Hangeul_SignExtKey1=1;
			else if(c==76) Hangeul_SignExtKey1=2;
			else if(c==58) Hangeul_SignExtKey1=3;
	...
	if(Ko_type=='3-2011' || Ko_type=='3-2012') {
		if((c==118 || c==56) && ( !prev_phoneme.length&&(!ohiQ[0]&&!ohiQ[3] || ohiQ[3]) || prev_phoneme.length&&unicode_cheot.indexOf(prev_combined_phoneme[0])<0&&(unicode_ga.indexOf(prev_combined_phoneme[0])>=0 || unicode_ggeut.indexOf(prev_combined_phoneme[0])>=0)) ) {
...
function CGG_yesHangeul(f,c,cc) {	// 세벌식 옛한글 처리
	...
	if(prev_combined_phoneme[0]==0x1160 && unicode_cheot.indexOf(prev_combined_phoneme[1])>=0 && unicode_CGG_hangeul_phoneme.indexOf(cc)>=0 ) {
	...
	// 끝소리가 들어왔을 때
		...
		else if(unicode_cheot.indexOf(prev_phoneme[0])<0 && unicode_ga.indexOf(prev_phoneme[0])<0/* && prev_combined_phoneme[0]!=0x115F && prev_combined_phoneme[0]!=0x1160*/) {
	...
	if(prev_combined_phoneme.length && unicode_CGG_hangeul_phoneme.indexOf(cc)>=0) {
		ohiSelection(f,prev_combined_phoneme.length);
	}
}
...
function Hangeul_Sin3(f,e,c) { // 신세벌식
	...
	var Sin3_extended_sign_layout = typeof current_layout.extended_sign_layout != 'undefined' ? current_layout.extended_sign_layout : typeof K3_Sin3_P_extended_sign_layout != 'undefined' ? K3_Sin3_P_extended_sign_layout : null;
	...
	if(no_shift(c)) {
		...
		cc=convert_into_ohi_hangeul_phoneme(Sin3_layout[c-33]);
		cc2=convert_into_ohi_hangeul_phoneme(Sin3_layout[c-33+32]);	// 아랫글 자리
	}
	...
	if(option.enable_sign_ext && Hangeul_SignExtKey1 && Sin3_extended_sign_layout) {
	...
	else if((ohiRQ[3] || backup_ohiRQ[3]) && cc<31 && prev_phoneme[0]==0x119E && !(prev_phoneme.length>1 && (unicode_ga.indexOf(prev_phoneme[1])>=0 || unicode_ggeut.indexOf(prev_phoneme[0])>=0))) {
...
function CGG_Hangeul_Sin3(f,e,c) { // 첫가끝 방식으로 조합하는 신세벌식 한글 처리
	...
	var Sin3_layout=current_layout.layout;
	var Sin3_sublayout=typeof current_layout.sublayout != 'undefined' ? current_layout.sublayout : null;
	var Sin3_extended_sign_layout = typeof current_layout.extended_sign_layout != 'undefined' ? current_layout.extended_sign_layout : K3_Sin3_P_extended_sign_layout;
	...
	if(no_shift(c)) {
		...
		cc=convert_into_unicode_hangeul_phoneme(Sin3_layout[c-33]);
		cc2=convert_into_unicode_hangeul_phoneme(Sin3_layout[c-33+32]); // 아랫글 자리
	}
	...
	else if(option.enable_Sin3_diphthong_key && no_shift(c) && prev_phoneme.length && unicode_cheot.indexOf(prev_phoneme[0])>=0 && Sin3_sublayout && unicode_ga.indexOf(Sin3_sublayout[c-33])>=0) {
	// 첫소리가 들어갔고 가운뎃소리가 들어가지 않았을 때 보조 배열의 겹홀소리 조합용 홀소리를 넣음
	...
	// 왼손 쪽 아랫글 자리에서 가운뎃소리 넣기
		cc=cc2;
		if(c==122 && (cc2==0x119E || unicode_CGG_hangeul_phoneme.indexOf(cc2)<0)) cc=0x119E; // Z 자리 아래아
	}
...
function show_options() {
	...
	if(opts) {
		...
		if(current_layout.type_name.substr(0,5)=='Sin3-' && typeof current_layout.extended_hangeul_combination_table != 'undefined') opt.style.display = 'block';
		else opt.style.display = 'none';
	...
	if(f.type=='text' && n=='INPUT' || n=='TEXTAREA') {
		if((c==13 || c==32) && !e.ctrlKey && !e.shiftKey && !e.altKey) {
			...
			if(Ko_type.substr(0,3)=='3m-' && !option.force_normal_typing) {
				if(e.shiftKey) {
				// 모아치기 자판이면서 윗글쇠가 함께 눌렸다면 이미 들어간 글쇠 정보를 먼저 처리하고 새로 넣음
					ohiHangeul3_moa(f,e);
					pressing_keys=0;
					pressed_keys=[];
					ohiHangeul3(f,e,c);
